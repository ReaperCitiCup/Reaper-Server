...
def fundGroup(codeList,pencentage):
    ...
    while(curDays<days):
        ...
        for i in range(len(codeList)):
            while(index[i] <len(fundDict[codeList[i]].date) and fundDict[codeList[i]].date[index[i]] > curTime.strftime('%Y-%m-%d')):
                index[i]+=1
...
def test2(code,startTime,endTime,option):   #参数：基金的代码，查询的起始和结束日期（'yyyy-mm-dd'的字符串类型,option为字符串，选择查看哪一个因子
     ...
     if('alpha'==option):
          ...
          while(i<=days):
               ...
               #print "temp.fundRate=",temp.fundRate[1:20],"\n\n\ntemp.rf=",temp.rf[1:20],"\n\n\ntemp.rm=",temp.rm
               ...
               if('stockSelectionCoefficient'==option):
                    print curEndTime.strftime('%Y-%m-%d')+"对应的择股系数为",linreg.intercept_
                    print "市场收益率为:",sum(temp.rm)
                    
               else:
                    print curEndTime.strftime('%Y-%m-%d')+"对应的择时系数为",linreg.coef_[1]*rmSubSqareRfAvg
                    print "市场收益率为:",sum(temp.rm)
               i+=30
          return
     ...
     while(i<=days):      
          ...
          if('beta'==option):              
               ...
               print curEndTime.strftime('%Y-%m-%d')+"对应的特雷诺指数为",treynorRatio[-1]
                             
          i+=1


#基金组合测试函数
def fundGroupTest(codeList,pencentage,startTime,endTime,option):
     code='myFundGroup'
     fundDict={}
...
#test2('000003','2007-09-08','2017-09-08',optionList[9])
#test()
codeList=['000003','000004']
pencentage=[0.5,0.5]
fundGroupTest([0.5,0.5],pencentage,'2007-09-08','2017-09-08',optionList[0])
